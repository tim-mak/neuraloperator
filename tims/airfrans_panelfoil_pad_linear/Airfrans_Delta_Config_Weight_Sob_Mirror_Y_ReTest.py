...
class AirfransOptConfig(OptimizationConfig):
    n_epochs: int = 2800  # check cli arguments
    learning_rate: float = 5e-4
    training_loss: str = "weighted_l2"
    weight_decay: float = 1e-4
    scheduler: str = "CosineAnnealingLR"
    scheduler_T_max: int = 3000 
...
class AirfransDatasetConfig(ConfigBase):
    ...
    channel_dim: int = 1
    weights: List[float] = [0.1, 1.0, 1.0, 0.5]  # Weights for [delta_Cp, delta_U_x, delta_U_y, log_nut_ratio] in loss calculation
    model_storage_dir: str = "/home/timm/storage/AF_NO_DATASET/ReTest_Model_FNO_Weight_Sobolev_mirrorY_linear_Modes_128_32_V1"
...
class Default(ConfigBase):
    ...
    model: ModelConfig = FNOConfig(
        ...
        n_modes=[128,32],   #  ( 64 real modes in X and 16 in Y after mirror) says  X direction is full complex fft needs 2N real modes # mirror padding means double the modes in Y direction to account for the padding (256,32) -> (256,64)
        hidden_channels=32,
