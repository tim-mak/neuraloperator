...
def plot_latent_space(model_iphi, physical_mesh,ep, code=None):
    model_iphi.eval()
    with torch.no_grad():
        # physical_mesh shape: (1, N_points, 2) -> [x, y]
        # output shape: (1, N_points, 2) -> [xi, eta]
        latent_coords = model_iphi(physical_mesh.cuda(), code=code)
        latent_coords = latent_coords.cpu().numpy().squeeze()
        physical_coords = physical_mesh.detach().cpu().numpy()

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))

    # Plot 1: Physical Space (The warped C-mesh)
    # We plot a subset of lines to see the "grid"
    ax1.scatter(physical_coords[:, 0], physical_coords[:, 1], s=1, c='blue', alpha=0.5)
    ax1.set_title("Physical Space (x, y)")
    ax1.set_aspect('equal')

    # Plot 2: Latent Space (The "un-warped" logical grid)
    ax2.scatter(latent_coords[:, 0], latent_coords[:, 1], s=1, c='red', alpha=0.5)
    ax2.set_title("IPHI Latent Space (ξ, η)")
    ax2.set_aspect('equal')

    plt.savefig(f'latspace_ep{ep}.png')
...
if __name__ == "__main__":
    ...
    ################################################################
    PATH_Sigma = '../data/elas/Random_UnitCell_sigma_10.npy'
    PATH_XY = '../data/elas/Random_UnitCell_XY_10.npy'
    PATH_rr = '../data/elas/Random_UnitCell_rr_10.npy'
    ...
    for ep in range(epochs):
        ...
        if ep%100==0:
            ...
            plot_latent_space(model_iphi, mesh, ep, code=None)
