...
def test_hybrid_padder_real_data(data_file, sample_idx=0):
    ...
    try:
        y_tensor = data['y'][sample_idx:sample_idx+1,:, :, :]
    except KeyError:
        print("Error: Dataset does not contain the 'y' key.")
        return
        
    _, Nc, Nx, Ny = y_tensor.shape
    
    # Initialize Hybrid Padder
    x_axis_strategy=['legendre', 'legendre', 'legendre', 'taper', 'taper']
    padder = HybridChannelWiseSpectralPadder(d=2, n_additional_pts=50, x_axis_strategy = x_axis_strategy).to(y_tensor.device)
    ...
    out_dir = "/home/timm/storage/AF_NO_DATASET/C_mesh_V2/PaddingTest"
    os.makedirs(out_dir, exist_ok=True)
    for ch in range(Nc):   

        # Convert back to numpy (squeeze out B and C dims)
        Z_orig = y_tensor[0, ch].detach().cpu().numpy()
        Z_pad = y_padded[0, ch].detach().cpu().numpy()
        ...
        fig.suptitle(f"Hybrid Padder Test: Sample {sample_idx}, Channel: {ch}", fontsize=16, fontweight='bold')
        ...
        plt.tight_layout()
        outfile = os.path.join(out_dir, f"hybrid_padder_test_Y_channel_{ch}.png")
        plt.savefig(outfile, dpi=150)
        print(f"Saving channel {ch} to {outfile}")
        plt.show()
        plt.close(fig)
    
    plt.close('all')

test_hybrid_padder_real_data("/home/timm/storage/AF_NO_DATASET/C_mesh_V2/train/airfoil_aoa_train_CMesh_V2_256x64.pt", sample_idx=200)