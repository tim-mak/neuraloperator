...
def pyvista_plot_airfrans_scalar(grid, scalar_name="Scalar", scalar_min=-1.0, scalar_max=1.0):
    ...
    plotter.add_mesh(
        ...
        cmap='coolwarm',     
        show_edges=True, 
        edge_opacity=0.5,  # Set to True if you want to see the rectilinear cells
...
def convert(dataset_root, output_folder, xlen, ylen, xoffset, grid_size):
    ...
    for sim_dir in sim_dirs[19:20]:  # Process only the 5th simulation for testing
        ...
        z_cst_snap[1:-1], t_snap[1:-1] = cst_airfoil.snap_kt_to_bspline_along_normal(z_kt_foil[1:-1], kt_normals[1:-1], t_init=t_init[1:-1], t_delta=1/points_on_foil, t_min=0.0, t_max=1.0)
        ...
        sdf_raw = sampled.point_data['implicit_distance'].reshape(z_grid_smooth.shape,order='C')
        u_raw = sampled.point_data['U'][:, 0].reshape(z_grid_smooth.shape,order='C')
        v_raw = sampled.point_data['U'][:, 1].reshape(z_grid_smooth.shape,order='C')
        p_raw = sampled.point_data['p'].reshape(z_grid_smooth.shape,order='C')
        nut_raw = sampled.point_data['nut'].reshape(z_grid_smooth.shape,order='C')
        ...
        sampled.point_data['cp_cfd'] = cp_cfd.flatten()
        ...
        pyvista_plot_airfrans_scalar(sampled, scalar_name="cp_cfd", scalar_min=-1.0, scalar_max=1.0)
        
        pyvista_plot_airfrans_scalar(sampled, scalar_name="vtkValidPointMask", scalar_min=0, scalar_max=1.0)

        print(f" Cp_cfd shape {cp_cfd.shape} Cp_pot shape {cpz.shape} ")
        ...
        # Delta Cp between CFD and potential flow

        print(f" Cp_cfd shape {cp_cfd.shape} Cp_pot shape {cp_pot.shape} ")
        cp_diff = cp_rans - cp_pot
        sampled.point_data['cp_diff'] = cp_diff
        pyvista_plot_airfrans_scalar(sampled, scalar_name="cp_diff", scalar_min=-0.2, scalar_max=0.2)
