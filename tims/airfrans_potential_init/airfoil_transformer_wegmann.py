...
class AirfoilTransformer:
    def __init__(self,coords):
        ...
        self.z1 = self.coords[:,0] + 1j * self.coords[:,1]
        ...
        # calculat the complex centroid
        #self.centroid = np.mean(self.z1)
        #print(f"Airfoil centroid at: ({self.centroid.real:.6f}, {self.centroid.imag:.6f})")


        #self.z1 = self.z1 * (4.0 / x_span)  #

        # Force the TE to be exactly at z=2.0
        #self.z1[0] = 2.0 + 0j
        #self.z1[-1] = 2.0 + 0j
        ...
        centroid_z2 = torch.mean(self.z2)
        self.z2 = self.z2 - centroid_z2
        ...
        self.coeffs, zeta_final = self.fornberg_outer_iteration(self.z2, n_iter=10, tol=1e-12)

        self.validate_mapping(zeta_final, self.coeffs)


    def fornberg_outer_iteration(self, zeta, n_iter=10, tol=1e-10):
        """
