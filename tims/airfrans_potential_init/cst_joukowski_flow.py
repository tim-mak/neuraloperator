...
from foil_utils import CSTAirfoil, sort_airfoil_to_sellig
import airfrans as af
from scipy.optimize import minimize

from mesher import  ConformalGridGenerator
...
def convert(dataset_root, output_folder, xlen, ylen, xoffset, grid_size):
    ...
    for sim_dir in sim_dirs[4:5]:  # Process only the 5th simulation for testing
        ...
        # plt.plot(foil.points[:, 0], foil.points[:, 1], 'k.-')
        # plt.title(f"Original Airfoil Points: {name}")
        # plt.xlabel("x")
        # plt.ylabel("y")
        # plt.axis('equal')
        # plt.grid()
        ...
        conformal_grid_generator = ConformalGridGenerator(x_foil, y_foil)
        x_grid, y_grid = conformal_grid_generator.generate_conformal_grid(n_radial=100, far_field_radius=5.0)

        pyvista_mesh(x_grid, y_grid)
