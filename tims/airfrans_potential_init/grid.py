...
def airfoil():
    ...
    x3_n = int(15/80*xn)
    x4_n = int(26/80*xn)
    x1 = np.linspace(1, p, x1_n, endpoint=False)
    x2 = np.linspace(p, 0, x2_n, endpoint=False)
    x3 = np.linspace(0, p, x3_n, endpoint=False)
    x4 = np.linspace(p, 1, x4_n)
    x  = np.concatenate((x1, x2, x3, x4))

    yt = 5*t * (0.2969*x**.5 - 0.1260*x - 0.3516*x**2 + \
            0.2843*x**3 - 0.1015*x**4)
    print(f"Max thickness: {np.max(yt):.6f} at x = {x[np.argmax(yt)]:.6f}")
    print(f"xn: {xn}")
    print(f"p: {p}")
    yc  = np.zeros(xn)  
    dyc = np.zeros(xn)
