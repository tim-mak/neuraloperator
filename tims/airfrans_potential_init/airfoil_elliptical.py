...
def plot_surface_cp(sx, sy, cp_surface, title="Surface Pressure Distribution"):
    # 1. Normalize x-coordinate to [0, 1] for the plot
    x_min, x_max = sx.min(), sx.max()
    x_norm = (sx - x_min) / (x_max - x_min)
    
    # 2. Split into Upper and Lower surfaces 
    # Based on your O-grid wrap (0 is TE, middle is LE, end is TE)
    mid_idx = len(sx) // 2
    
    plt.figure(figsize=(10, 6))
    
    # Plotting Upper and Lower separately helps verify the TE join
    plt.plot(x_norm[:mid_idx], cp_surface[:mid_idx], 'r-', label='Surface Segment A')
    plt.plot(x_norm[mid_idx:], cp_surface[mid_idx:], 'b--', label='Surface Segment B')
    
    # 3. Aesthetics
    plt.gca().invert_yaxis() # Standard Aero convention: -Cp is up
    plt.axhline(0, color='black', lw=1, ls=':')
    plt.xlabel('Normalized Chord (x/c)')
    plt.ylabel('Pressure Coefficient (Cp)')
    plt.title(f"{title}\nTE Stagnation check: {cp_surface[0]:.4f}")
    plt.legend()
...
class PotentialFlowField:
    ...
    def solve_surface_only(self, grid_coords, metrics, alpha_deg=5.0):

        self.V_inf = 1.0  # Ensure V_inf is set for the Kutta condition
        # 1. Extract surface points (eta=0)
        # Shape is (512,)
        sx = grid_coords[:, 0, 0]
        sy = grid_coords[:, 0, 1]
        
        # 2. Canonical Surface Physics (The Unit Circle)
        theta = np.linspace(0, 2*np.pi, len(sx))
        alpha = np.radians(alpha_deg)
        
        # Kutta Condition: Gamma moves stagnation to theta_te
        theta_te = get_te_angle(grid_coords[:,:,0], grid_coords[:,:,1])
        theta_te = 0.0
        gamma = 4 * np.pi * self.V_inf * 1.0 * np.sin(alpha + theta_te)
        
        # Tangential velocity on a cylinder: V_theta = -2*Vinf*sin(theta-alpha) - gamma/(2*pi*R)
        v_theta_can = -2 * self.V_inf * np.sin(theta - alpha) - gamma / (2 * np.pi * 1.0)
        
        # 3. Apply the Metric 'Stretch'
        # h_xi is the distance between points on the airfoil per unit index
        x_xi = np.gradient(sx) 
        y_xi = np.gradient(sy)
        h_xi = np.sqrt(x_xi**2 + y_xi**2)
        
        # Safety: h_xi is the meter-per-index stretch. 
        # v_theta_can is m/s. We need to normalize by the 'index-to-radian' step.
        d_theta_d_xi = (2 * np.pi) / (len(sx) - 1)
        

        # .25 = chord (unit circle) 4 / 1  
        # Physical tangential velocity
        u_tangential_phys = v_theta_can * (d_theta_d_xi / h_xi) * 0.25  
        
        # 4. Surface Cp
        cp_surface = 1.0 - (u_tangential_phys**2 / self.V_inf**2)

        print(f"DEBUG: Surface Cp range: {cp_surface.min():.4f} to {cp_surface.max():.4f}")


        plot_surface_cp(sx, sy, cp_surface, title=f"Surface Cp Distribution at AoA {alpha_deg}°")
        
        return sx, sy, cp_surface
    
    def solve_2d_mapped(self, grid_coords, metrics, alpha_deg=5.0):
        # grid_coords shape (512, 40) -> (N_xi, N_eta)
        n_xi, n_eta = grid_coords.shape[0], grid_coords.shape[1]
        
        # 1. Coordinate Setup (Canonical Domain)
        # Map indices directly to a uniform polar space
        theta = np.linspace(0, 2*np.pi, n_xi)
        r_min = 1.0 # Unit cylinder
        # Map the last layer of your physical grid to the canonical radius
        r_max = 1.0 + (np.linalg.norm(grid_coords[0,-1,:2]) / np.linalg.norm(grid_coords[0,0,:2]))
        r = np.linspace(r_min, r_max, n_eta)

        T_mesh, R_mesh = np.meshgrid(theta, r, indexing='ij')
        z = R_mesh * np.exp(1j * T_mesh)
        
        # 2. Physics & Kutta Condition
        alpha = np.radians(alpha_deg)
        # Use your robustly detected TE angle to lock the stagnation point
        theta_te = 0 #get_te_angle(grid_coords[:,:,0], grid_coords[:,:,1])
        # Gamma required to move stagnation point to TE:
        gamma_kutta = 4 * np.pi * self.V_inf * r_min * np.sin(alpha + theta_te)
        
        # 3. Complex Velocity (dw/dz) in Circle Space
        # This represents the conjugate of the velocity in the canonical plane
        dw_dz = self.V_inf * (np.exp(-1j * alpha) - (r_min**2 * np.exp(1j * alpha)) / z**2) + \
                (1j * gamma_kutta) / (2 * np.pi * z)
        
        
        # 3. Project to Computational Gradients (phi_xi, phi_eta)
        # These represent 'Change per Index Step'
        v_can_x = np.real(dw_dz)
        v_can_y = -np.imag(dw_dz)
        
        # Units: (m/s) * (rad/index) -> Change/Index
        phi_xi_can = v_can_x * (2 * np.pi / (n_xi - 1)) 
        phi_eta_can = v_can_y * ((r_max - r_min) / (n_eta - 1))

        # 4. The Calibrated Bridge
        # Use the scale factor that worked on the surface
        scale = (np.max(grid_coords[:,:,0]) - np.min(grid_coords[:,:,0])) / 4.0
        inv_J = 1.0 / np.clip(metrics['J'], 1e-9, None)
        ...
        x_eta = metrics['x_eta']
        x_xi = metrics['x_xi']
        print(f"DEBUG: Metrics shapes - y_eta: {y_eta.shape}, y_xi: {y_xi.shape}, x_eta: {x_eta.shape}, x_xi: {x_xi.shape}")
        
        u_phys = inv_J * (y_eta * phi_xi_can - y_xi * phi_eta_can) * scale
        v_phys = inv_J * (-x_eta * phi_xi_can + x_xi * phi_eta_can) * scale

        cp = 1.0 - (u_phys**2 + v_phys**2) / (self.V_inf**2)

        print(f"DEBUG: U_phys range: {u_phys.min():.4f} to {u_phys.max():.4f}")
        print(f"DEBUG: V_phys range: {v_phys.min():.4f} to {v_phys.max():.4f}")
        print(f"DEBUG: Cp range: {cp.min():.4f} to {cp.max():.4f}")
        
        return grid_coords[:,:,0], grid_coords[:,:,1], u_phys, v_phys,cp    
    ...
    def solve_on_mapped_grid(self, grid_coords, metrics, alpha_deg=5.0):
        ...
        # Create the Complex Z-plane for the circle
        T_mesh, R_mesh = np.meshgrid(theta, r, indexing='ij')
        z = R_mesh * np.exp(1j * T_mesh)
        ...
        #v_can_conj = dw_dz 
        #phi_xi_can = np.real(v_can_conj) 
        #phi_eta_can = -np.imag(v_can_conj)

        v_can_x = np.real(dw_dz)
        v_can_y = -np.imag(dw_dz)

        phi_xi_can = v_can_x * (2 * np.pi * R_mesh) * (1.0) 
        phi_eta_can = v_can_y * (r_max - r_min) * (1.0)

        # Canonical Cp (This should be perfectly stable: -3.0 to 1.0)
        v_can_mag_sq = phi_xi_can**2 + phi_eta_can**2
        cp_can = 1.0 - (v_can_mag_sq / self.V_inf**2)

        print(f"DEBUG: dw/dz range: {dw_dz.min():.4f} to {dw_dz.max():.4f}")
        print(f"DEBUG: phi_xi_can range: {phi_xi_can.min():.4f} to {phi_xi_can.max():.4f}"  )
        print(f"DEBUG: phi_eta_can range: {phi_eta_can.min():.4f} to {phi_eta_can.max():.4f}")
        print(f"DEBUG: Cp in canonical space range: {cp_can.min():.4f} to {cp_can.max():.4f}")


        # 4. The Scaling Bridge (Jacobian Projection)
        # Extract pre-calculated metrics
        J = metrics['J']
        J_mean = np.mean(np.abs(metrics['J']))
        inv_J = 1.0 / (metrics['J'] + 1e-3 * J_mean)

        x_xi, x_eta = metrics['x_xi'], metrics['x_eta']
        y_xi, y_eta = metrics['y_xi'], metrics['y_eta']


        #|
        # Apply safety floor to J to prevent 10^9 spikes
        #J_safe = np.clip(J, 1e-9, None)

        print(f"DEBUG: J range before clipping: {J.min():.2e} to {J.max():.2e}")
        print(f"DEBUG: J mean  {J_mean:.2e} to {(J_mean + 1e-3 * J_mean):.2e}")
        
        # Transform canonical gradients to physical velocities
        # This maps the 'Circle' flow onto the 'Airfoil' mesh
        u_phys = (y_eta * phi_xi_can - y_xi * phi_eta_can) * inv_J
        v_phys = (-x_eta * phi_xi_can + x_xi * phi_eta_can) * inv_J
...
def validate_mapping_metrics(grid_coords, metrics):
    """
    Attempts to reconstruct the unit logical steps (d_xi, d_eta) 
    from physical coordinates using the provided metrics.
    """
    # 1. Extract Metrics
    x_xi, x_eta = metrics['x_xi'], metrics['x_eta']
    y_xi, y_eta = metrics['y_xi'], metrics['y_eta']
    ...
    # 2. Compute Inverse Metrics (The 'Pull-back')
    ...
    # In a perfect mapping, (xi_x * x_xi + xi_y * y_xi) should equal 1.0 (d_xi/d_xi)
    reconstruction_xi = xi_x * x_xi + xi_y * y_xi
    reconstruction_eta = eta_x * x_eta + eta_y * y_eta
    
    # 4. Error Analysis
    error_map = np.abs(reconstruction_xi - 1.0) + np.abs(reconstruction_eta - 1.0)
    
    return error_map, reconstruction_xi, reconstruction_eta
...
def calculate_metrics(grid):
    # n_xi = 512, n_eta = 40
    n_xi, n_eta = grid.dimensions[0], grid.dimensions[1]
    pts = grid.points.reshape(n_xi, n_eta, 3)
    
    # 1. Standardize computational steps to [0, 1]
    d_xi = 1.0 / (n_xi - 1)
    d_eta = 1.0 / (n_eta - 1)
    
    # 2. Compute derivatives normalized by unit step
    # This makes x_xi and y_eta 'Meters per Unit-Computational-Length'
    grad_x = np.gradient(pts[:,:,0])
    grad_y = np.gradient(pts[:,:,1])
    
    x_xi = grad_x[0] / d_xi
    x_eta = grad_x[1] / d_eta
    y_xi = grad_y[0] / d_xi
    y_eta = grad_y[1] / d_eta
    
    # 3. Jacobian (Should be positive for O-grids)
    # If it's negative, your indexing order (xi vs eta) is reversed
    jacobian = x_xi * y_eta - x_eta * y_xi
    
    # Check for negative area and flip if necessary to stay physically consistent
    #if np.mean(jacobian) < 0:
    #    jacobian = -jacobian
    ...
    return jacobian, (x_eta**2 + y_eta**2), (x_xi**2 + y_xi**2), metrics
...
def convert(dataset_root, output_folder, xlen, ylen, xoffset, grid_size):
    ...
    for sim_dir in sim_dirs[4:5]:
        ...
        # --- Diagnostic Plotting ---
        error_map, rec_xi, rec_eta = validate_mapping_metrics(grid_data, metrics)

        plt.figure(figsize=(12, 5))
        plt.subplot(1, 2, 1)
        plt.imshow(error_map.T, origin='lower', aspect='auto', cmap='inferno')
        plt.colorbar(label='Metric Orthogonality Error')
        plt.title("Localized Mapping Failure Points")

        plt.subplot(1, 2, 2)
        plt.hist(error_map.flatten(), bins=100, log=True)
        plt.title("Error Distribution (Log Scale)")
        plt.xlabel("Deviation from Identity")
        plt.show()
        ...
        #sx,sy,cp_surface = potflow.solve_surface_only(grid_data, metrics, alpha_deg=aoa_deg)

        x, y, U_phys, V_phys, Cp_phys = potflow.solve_2d_mapped(grid_data, metrics, alpha_deg=aoa_deg)

        #grid_coords[:,:,0], grid_coords[:,:,1], u_phys, v_phys, cp
        #x, y, U_phys, V_phys, Cp_phys = potflow.solve_on_mapped_grid(grid_data, metrics, alpha_deg=aoa_deg)

        


        # grid.point_data["Jacobian"] = jac.ravel()  #(Cell Area Metric)
        # grid.point_data["Alpha"] = alpha.ravel()  #(Radial Stretch)
        # grid.point_data["Gamma"] = gamma.ravel()  #(Circumferential Stretch)
        ...
        # visualize_pyvista(grid_data, {"Jacobian": jac.ravel()      ,
        #                                "Alpha": alpha, 
        #                                "Gamma": gamma, 
        #                                "Cp": Cp_phys.flatten(),
        #                                "U": U_phys.flatten(),
