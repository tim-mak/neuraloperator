...
class SelectiveUnitGaussianNormalizer(UnitGaussianNormalizer):
    def __init__(self,  dim, eps=1e-5, channels_to_normalize=[0, 1, 3, 4], mask_channel=2):
    ...
    def transform(self, x):
        # x is [Batch, 5, H, W]       
        # 5 channel input is [u_input, v_input, mask_binary, sdf_fixed, log_nu]
        # Ensure mean and std are specifically the same shape as the slice
        # we want to normalize all channels except the mask (channel 2)
        ...
        # If the input is 5 channels but we only want to normalize 4 of them
        else:
...
class AirfoilDataProcessor(DefaultDataProcessor):
    def __init__(self, in_normalizer=None, out_normalizer=None, device='cuda'):
        """
        Extends DefaultDataProcessor to handle the airfoil mask 'Hard Gate'.
        in_normalizer: Normalizes [u_inf, v_inf, mask, sdf]
        out_normalizer: Normalizes [u, v, p, nut] if in training mode, returns physical units if in eval mode.
        """
    ...
    def preprocess(self, data, batched=True):
        ...
        # 2. THE HARD GATE [cite: 2026-02-05]
        # Input Channel 2 is the mask. The normalizer may have turned 0/1 
        # into floats (e.g., -1.2 to 0.8). We force it back to a sharp 0/1.
        # We also flip it if your FNO convention requires 1 for airfoil/0 for flow.
        raw_mask = data['x'][:, 2:3, :, :].to(self.device)
        x[:, 2:3, :, :] = 1.0 - raw_mask # 1.0 = Airfoil, 0.0 = Flow
    ...
    def postprocess(self, output,data_dict):
        ...
        - Eval: Returns de-normalized units (Cp, u, v, nut) for the Evaluator.
        """
...
class AirfoilEvaluator:
    def __init__(self, processor, device='cuda'):
        ...
        Args:
            processor: Your AirfoilDataProcessor (DefaultDataProcessor) 
                       containing the trained normalizers.
...
class AirfransDataset(PTDataset):
    def __init__(
        ...
        if encode_input:
            ...
            # Channels: [u_input, v_input, mask_binary, sdf_linear, log_nu]
            #           [   0   ,    1   ,      2     ,    3    ,    4    ]
            channels_to_normalize = [0, 1, 3, 4]  # Skip channel 2 (mask_binary)
            ...
            input_encoder.mean[:, input_encoder.mask_channel, ...] = 0.0
            input_encoder.std[:, input_encoder.mask_channel, ...] = 1.0
        ...
        self._data_processor = AirfoilDataProcessor(
        ...
        for res, n_test, test_split in zip(test_resolutions, n_tests, test_splits):
            ...
            test_file = data_dir / f"{dataset_name}_{test_split}_{res}x{res}.pt"
...
def plot_model_predictions(model, dataset, data_processor, sample_idx=0, device='cuda'):
    ...
    output_names = ['u_delta_norm', 'v_delta_norm', 'Cp', 'log_nut_ratio']
...
def plot_sample_comparison(dataset, sample_idx=0, figsize=(15, 10), save_path=None):
    ...
    input_channels = ['u_input', 'v_input', 'mask_binary', 'sdf_linear', 'log_nu']
    output_channels = ['u_delta_norm', 'v_delta_norm', 'Cp', 'log_nut_ratio']
...
if __name__ == "__main__":
    ...
    data_root = "/home/timm/Projects/PIML/Dataset_PT_FNO/TrainingX5Y4_consolidated"

    get_dataset_stats(data_root + "/airfoil_full_train_128x128.pt")
