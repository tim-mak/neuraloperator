...
class AirfransDeltaTrainer(Trainer):
    ...
    def plot_diagnostic_grid(self, loader, epoch, save_dir="plots", sample_idx=0, prefix="prediction",training_loss=None):
        ...
        with torch.no_grad():
            ...
            # 1. Preprocess: This mirrors 32 -> 64 and pads 256 -> 512
...
#
        ...
        fig.suptitle(f"Airfrans Prediction: delta Cp , delta U, delta V, log(nut/nu) : Epoch {epoch}: Sample {sample_idx}", fontsize=20)
        n_labels = [r'$\Delta C_p$', r'$\Delta U_x$', r'$\Delta U_y$', r'$\log_{10}(\nu_t/\nu)$']
        ...
        for i in range(n_out):
            ...
            #im1 = axes[i, 1].imshow(y_norm_truth[0, i].cpu().numpy().T, origin='lower', cmap='plasma', vmin=z_min, vmax=z_max)
