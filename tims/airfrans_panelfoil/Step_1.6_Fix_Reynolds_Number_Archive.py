...
def correct_reynolds_number(
    ...
    for file in data_root.glob("*.pt"):
        try:
            data = torch.load(file)
            props = data.get('props', {})
            if 'reynolds' in props:
                
                original_re = props['reynolds']
                right_re = props['v_mag_inf'] / 1.56e-5

                if not np.isclose(original_re, right_re, rtol=0.01):
                    print(f"Warning: Original Reynolds number {original_re} in {file.name} does not match expected value {right_re:.2e} based on v_mag_inf and kinematic viscosity. Correcting it.")
                    corrected_re = right_re
                    props['reynolds'] = corrected_re
                    props['log_reynolds'] = np.log(corrected_re)
                    data['props'] = props


                    torch.save(data, file)
                    print(f"Updated Reynolds number in {file.name}: {original_re} -> {corrected_re}")
                else:
                    print(f"Reynolds number in {file.name} is correct: {original_re}")
            else:
                print(f"No Reynolds number found in props of {file.name}")
        except Exception as e:
            print(f"Error processing {file.name}: {e}")
            print(f"Error: Archive directory not found at {data_root}")
...
def main():
    ...
    new_data_root = Path("/home/timm/storage/AF_NO_DATASET/ArchiveRn")
    ...
    new_data_root = data_root / "ArchiveRn"
    new_data_root.mkdir(parents=True, exist_ok=True)
    correct_reynolds_number(data_root=data_root,
                             output_dir=new_data_root)
    # Run consolidation
