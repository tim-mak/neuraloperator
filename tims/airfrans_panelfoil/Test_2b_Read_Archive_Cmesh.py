...
def convert_pt_to_cmesh_pyvista(x, y_delta,y_out):
    ...
    cmesh.point_data["x_xi"]       = flat(x[6])
    cmesh.point_data["x_eta"]      = flat(x[7])
    cmesh.point_data["y_xi"]       = flat(x[8])
    cmesh.point_data["y_eta"]      = flat(x[9])
    cmesh.point_data["det_J"]      = flat(x[10])
...
if __name__ == "__main__":
    ...
    for name in names:
        ...
        for res in resolutions:
            ...
            y_out = raw_data["y"] # Output targets (delta/correction fields)
